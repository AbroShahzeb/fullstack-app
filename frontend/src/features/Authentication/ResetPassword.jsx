...
function ResetPassword({ user }) {
  ...
  const [resetPasswordStudent, { isLoading: isLoadingStudent }] =
    useResetPasswordStudentMutation();
  const [resetPasswordTeacher, { isLoading: isLoadingTeacher }] =
  ...
  async function onSubmit(data) {
    try {
      ...
      let res;
      if (user === "Student")
        res = await resetPasswordStudent(reqBody).unwrap();
      ...
      <div className="w-full xs:w-96 flex flex-col gap-8 items-center  mt-8 sm:mt-16 font-primary relative ">
        ...
        <div className="relative xs:w-full">
          ...
          >
            ...
            >
              {isLoadingTeacher || isLoadingStudent ? "Resetting..." : "Reset"}
            </button>
          </form>
        </div>
      </div>
